...
class MediaStream extends EventEmitter {
  ...
  start() {
    ...
    this.multistream = MultiStream(this.next.bind(this))
    .pipe(decoder)
    .pipe(encoder);

    _.forEach(this.clients, client => client.start(encoder));
  ...
  next(cb) {
    ...
    if (!this.hasMediaSource(item.getSource())) return this.next(cb);

    const mediasource = this.getMediaSource(item.getSource());

    const playable = mediasource.get(item);
    playable.on('end', () => {
  ...
  registerMediaSource(mediasource) {
    if (!(mediasource instanceof MediaSource)) throw new Error('The given mediasource is not an instance of MediaSource!');
   ...
   *
   * @param {Mixed} client Client id (String) or Client instance
   *
   * @return MediaSource this This instance
   */
   ...
   */
  getMultiStream() {
    if (_.isUndefined(this.multistream)) throw new Error('No multistream instance available! Did you forget to start the stream?');
